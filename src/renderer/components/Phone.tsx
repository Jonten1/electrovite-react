...
const Phone = ({ credentials, onLogout }: PhoneProps) => {
  ...
  const handleTransfer = async (targetExtension: string) => {
    ...
    try {
      ...
      const response = await fetch(
        `${window.electron.env.API_TRANSFER_PROTOCOL}://${window.electron.env.SERVER_URL}/transfer-call`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
      ...
      // Set up WebSocket handler for transfer status
      if (ws && ws.readyState === WebSocket.OPEN) {
        ws.send(
          JSON.stringify({
            type: 'callStatus',
      ...
      // End the current call once transfer is initiated
      session.terminate();
      setIsInCall(false);
